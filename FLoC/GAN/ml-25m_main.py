...
# Note: if change data need to change SEQ_LENGTH, vocab_size here
#  for file need to change positive file and negative file ?
#  note that realtest only used for eval_bleu.py ?

flags = tf.app.flags
FLAGS = flags.FLAGS
# flags.DEFINE_boolean('restore', False, 'Training or testing a model') # Note: py3 adapted repo put default to True
...
SEQ_LENGTH = 32 # sequence length # 32, 77 Note: need to modify that if change length of history in data ?
START_TOKEN = 0 # Note: conflict with what define as 0 in vocab ?
...
#########################################################################################
TOTAL_BATCH = 800
positive_file = 'save_ml25m/realtrain_ml25m_5000_32.txt' # 'save_ml25m/realtrain_ml25m.txt'
negative_file = 'save_ml25m/generator_sample.txt'
generated_num = 10000
model_path = './ckpts_ml25m'
# Note: more code here for py3 adapted as did some different things
...
def get_reward(model,dis, sess, input_x, rollout_num, dis_dropout_keep_prob,total_epoch,data_loader):
    ...
    for i in range(rollout_num):
        ...
        for given_num in range(1, model.sequence_length // model.step_size): # Note: int div new syntax
        ...
        if i == 0:
            ...
            rewards[model.sequence_length // model.step_size - 1] += ypred # Note: int div new syntax
...
def main():
    ...
    vocab_size = 5002 # 4839, 5847 Note: changed as otherwise crash in feature extractor
    ...
    discriminator = Discriminator(SEQ_LENGTH,num_classes=2,vocab_size=vocab_size,dis_emb_dim=dis_embedding_dim,filter_sizes=dis_filter_sizes,num_filters=dis_num_filters,
                        batch_size=BATCH_SIZE,hidden_dim=HIDDEN_DIM,start_token=START_TOKEN,goal_out_size=GOAL_OUT_SIZE,step_size=4) # Note: py3 adapted changed step_size to 1
    leakgan = LeakGAN(SEQ_LENGTH,num_classes=2,vocab_size=vocab_size,emb_dim=EMB_DIM,dis_emb_dim=dis_embedding_dim,filter_sizes=dis_filter_sizes,num_filters=dis_num_filters,
                        batch_size=BATCH_SIZE,hidden_dim=HIDDEN_DIM,start_token=START_TOKEN,goal_out_size=GOAL_OUT_SIZE,goal_size=GOAL_SIZE,step_size=4,D_model=discriminator) # Note: py3 adapted changed step_size to 1

    config = tf.ConfigProto()
    config.gpu_options.allow_growth = True
    config.gpu_options.per_process_gpu_memory_fraction = 0.5 # Note: py3 adapted commented out this line
    ...
    for a in range(1):
        ...
        print("epoch:",a,"  ") # () wrap print

    log = open('save_ml25m/experiment-log.txt', 'w')
    generate_samples(sess, leakgan, BATCH_SIZE, generated_num, negative_file, 0)
    gen_data_loader.create_batches(positive_file)
    saver_variables = tf.global_variables()
    # Note: max_to_keep=5 by default can increase if want more ckpt kept
    saver = tf.train.Saver(saver_variables, max_to_keep=None) # default max_to_keep=5
    model = tf.train.latest_checkpoint(model_path)
    print(f'latest checkpoint path: {model}') # () wrap print Note: this printed None after epoch: 0 ?
    if FLAGS.restore and model:
        # model = tf.train.latest_checkpoint(model_path)
        # if model and FLAGS.restore:
        if model_path+'/' + FLAGS.model: # Note: FLAGS.model is empty string so evaluate to tru but don't get any filepath ?
            # Note: maybe FLAGS.model can be set by sysargs passed to Main.py but not used anyway ?
            print(model_path+'/' + FLAGS.model) # () wrap print
            # saver.restore(sess, model_path+'/' + FLAGS.model) # Note: throw error
            model_path_correct = model.replace(os.sep, '/')
            print(f'checkpoint will load: {model_path_correct}')
            saver.restore(sess, model_path_correct)
        else:
            # saver.restore(sess, model)
            print(f'replacing {model} to:') # Note: the filepath was with the wrong separator
            model_path_correct = model.replace(os.sep, '/')
            print(f'{model_path_correct}')
        ...
        if FLAGS.resD and model_path + '/' + FLAGS.model:
                ...
                print('Start pre-training...') # () wrap print
                log.write('pre-training...\n')
                # for epoch in xrange(PRE_EPOCH_NUM):
                for epoch in range(PRE_EPOCH_NUM): # Note: switch to trange or others from tqdm ?
            ...
            # Note: was placed before in py3 adapted and modified ?
                print('Start pre-training discriminator...') # () wrap print
                # Train 3 epoch on the generated data and do this for 50 times
                for i in trange(16): # Note: py3 adapted changed to 8, changed range to trange or tqdm(range)
                    for _ in tqdm(range(5), leave=False): # Note: py3 adapted changed to 2
                        generate_samples(sess, leakgan, BATCH_SIZE, generated_num, negative_file,0)
                        # Note: py3 adpated had its own custom function split_sentence here
                        # gen_data_loader.create_batches(positive_file)
                        dis_data_loader.load_train_data(positive_file, negative_file)
                        for _ in tqdm(range(3), leave=False): # Note: py3 adapted changed to 2
        ...
        #  pre-train generator
                    print (f'Start pre-training... {i}/{16}')
                    log.write('pre-training...\n')
                    # for epoch in xrange(PRE_EPOCH_NUM/16):
                    # for epoch in range(PRE_EPOCH_NUM/16):
                    for epoch in trange(PRE_EPOCH_NUM//16): # Note: py3 adpated changed to 8
                        loss = pre_train_epoch(sess, leakgan, gen_data_loader)
                        if epoch % 5 == 0:
                            generate_samples(sess, leakgan, BATCH_SIZE, generated_num, negative_file,0)
    ...
    for total_batch in trange(TOTAL_BATCH, leave=False):
        # Train the generator for one step
        for it in trange(1, desc='G train', leave=False):

            for gi in range(gencircle): # Fast only iterate over 1
            # for gi in trange(gencircle, leave=False):
        ...
        if total_batch % 10 == 1 or total_batch == TOTAL_BATCH - 1:
            generate_samples(sess, leakgan, BATCH_SIZE, generated_num, "./save_ml25m/ml25m_" + str(total_batch) + ".txt", 0)
        ...
        for _ in trange(5, desc='D train', leave=False): # Note: py3 changed to 1
            generate_samples(sess, leakgan, BATCH_SIZE, generated_num, negative_file,0)
            dis_data_loader.load_train_data(positive_file, negative_file)

            for _ in range(3): # Note: py 3 changed to 2
            # for _ in tqdm(range(3), desc='D train', leave=False):
                dis_data_loader.reset_pointer()
                # for it in xrange(dis_data_loader.num_batch):
                for it in range(dis_data_loader.num_batch): # This is fast no need for trange ?
                # for it in trange(dis_data_loader.num_batch, leave=False):
                    x_batch, y_batch = dis_data_loader.next_batch()
                    feed = {
                        discriminator.D_input_x: x_batch,
                        discriminator.D_input_y: y_batch,
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f'Unexpected {sys.exc_info()[0]}')
        print(inst)
        raise
    finally:
        end_time = time()
        train_time = end_time - start_time
        (year, month, day, hour, min, sec, _, _, _) = gmtime(train_time)
        print(f'Overall trained model during {train_time:.2f} seconds or {hour}:{min}:{sec} (hours, minutes, seconds)')
